    ...
    rhs: &Vec<Box<dyn DataTypeConstraint>>) -> bool 
{
    ...
    c.convert_to_u8(&*lhs[0], lhs, &mut lhs_u8).expect("convert_from_u8 failed");
    c.convert_to_u8(&*rhs[0], rhs, &mut rhs_u8).expect("convert_from_u8 failed");
...
fn main() {
    ...
    println!("To enable debug output, add 'RUST_LOG=debug' before cargo run.");
